...
var idowsFileBrowser = new function () {
    ...
    this.init = ( options ) => {
        ...
        let windowmanager = this.tinyMCEWindowManager ();
        ( windowmanager === null ) && this.initializeBootstrapModals ();
    };
    ...
    this.dependencyCheck = ( options ) => {
        ...
        let isAssetsNull = typeof this.options.assetsDir === 'undefined';
        let windowManager = this.tinyMCEWindowManager ();
    ...
    this.toggleSettingsMenu = () => {
        ...
        if ( hasSelectedFiles ) {
            $ ( "#settings" ).parent ().addClass ( "idows-active" ).show ();
            return;
        }

        //hide settings
        $ ( "#settings-menu" ).hide ();
        $ ( "#settings" ).parent ().removeClass ( "idows-active" ).hide ();
    ...
    this.showMessage = ( message, type ) => {
        ...
        $ ( ".alert-" + type ).find ( '.message' ).html ( '<strong>' + message + '</strong>' );
        $ ( ".alert-" + type ).animate ( {
            top : 36 + 'px',
            zIndex : 999,
            right : 0,
    ...
    this.addContextMenuCommand = ( params, command ) => {
        ...
        //send request
        let commandPromise = this.sendRequest ( options );
    ...
    this.downloadFiles = ( files = null, formData = null ) => {
        ...
        let promiseDownload = this.sendRequest ( options );
    ...
    this.sendDeleteFileRequest = ( options ) => {
        ...
        deletePromise.then ( ( response ) => {
            if ( response.success ) {
                ...
                $ ( options.anchorLinks ).each ( function () {
    ...
    this.menuButtonBinding = () => {
        ...
        $ ( "#sort-menu" ).children ().children ().each ( function () {

            $ ( this ).on ( 'click', function () {
                let currentSortOption = $ ( this );

                //label span for the dropdown menus
                let label = $ ( this ).find ( 'span.idows-text' );
                let hasActiveClass = $ ( this ).hasClass ( 'idows-active' );
                ...
                siblings.each ( function () {
                    $ ( this ).find ( 'span.idows-text' ).text ( $ ( this ).find ( 'span.idows-text' ).attr ( 'data-name' ) + ' Ascending' );
    ...
    this.inProgress = () => {
        ...
        this.options.timeoutStore = window.setTimeout ( function () {
    ...
    this.uploadFinish = function ( response ) {
        ...
        if ( isFail ) {
            window.clearTimeout ( this.options.timeoutStore );
    ...
    this.switchView = function ( switchOptions ) {
        ...
        switchOptions = this.setDefaults ( switchOptions, defaultSwitchOptions );
        ...
        let switchViewEvents = {
            ...
            'list' : function () {
                ...
                $ ( '#fileview' ).idowsFileList ( {
                    ...
                    loadMessage : 'Loading file list please wait.'
                }, function ( file, li ) {
            ...
            'thumbnail' : function () {
                ...
                $ ( '#fileview' ).idowsFileList ( {
                    ...
                    sortBy : switchOptions.sortBy,
                    sortOrder : switchOptions.sortOrder,
                    loadMessage : 'Loading File Thumbs Please wait.'
                }, function ( file, li ) {
                    var filepath = file.split ( "/" );
                    var file_name = filepath[filepath.length - 1];
